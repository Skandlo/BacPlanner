...
class BacDatabase:
    ...
    def initialize_tables(self):
        ...
        with sqlite3.connect(self.db_name) as connection:
            ...
            # Table 3: Stores revision status for every lesson
            cursor.execute("""
    ...
    def populate_bac_info_subjects(self):
        """Inserts the Bac Informatique subjects and coefficients."""
    ...
    def populate_bac_info_lessons(self):
        ...
        lessons_data = [
            ...
            (
                "Sciences Physiques",
                "Phys - T2 - 1. LES OSCILLATIONS ELECTRIQUES FORCEES"
            ),
            ...
            (
                "Sciences Physiques",
                "Chimie - Rappel - 1. PHENOMENE D'OXYDOREDUCTION"
            ),

            (
                "Sciences Physiques",
                "Chimie - Rappel - 2. LES HYDROCARBURES ALIPHATIQUES"
            ),

            (
                "Sciences Physiques",
                "Chimie - T1 - 1. MESURE D'UNE QUANTITE DE LA MATIERE"
            ),
    ...
    def get_lessons_by_subject(self, subject_name):
        """Fetches all lessons and their current status."""
    ...
    def get_lessons_by_status(self, status=None, subject=None):
        """
        Returns lessons filtered by status and/or subject.

        status=None means all statuses.
        subject=None means all subjects.
        """
        ...
        with sqlite3.connect(self.db_name) as connection:
            ...
            query = """
                SELECT subject_name, lesson_title, status
                FROM revision_tracker
                WHERE 1=1
            """
            ...
            query += " ORDER BY subject_name, id"
    ...
    def get_status_counts(self):
        """Returns the number of lessons for each status."""
...
if __name__ == "__main__":
    ...
    for subject, coefficient in db.get_all_subjects():
        print(
            f"- {subject}: coefficient {coefficient}"
        )

    print("\nCurrent overall moyenne:")

    print(
        db.calculate_overall_moyenne()
    )

    print("\nExample Mathématiques lessons:")

    lessons = db.get_lessons_by_subject(
        "Mathématiques"
    )
